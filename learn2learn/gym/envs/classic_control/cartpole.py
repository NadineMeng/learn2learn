...
class MetaCartPoleEnv(gym.Env):#[np.ndarray, Union[int, np.ndarray]]):
    """
    ### Description
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element, it is not reflective of the allowed values of the state space in an unterminated episode. Particularly:
    ...
    def __init__(self,task={},n_tasks=1):
        self.gravity = 9.8
        self.masspole = 0.1
        self.masscart = 1.0
    ...
    def sample_tasks(self,num_tasks):
        lengths=np.random.uniform(0.45,0.55,size=(num_tasks,))
        forces=np.random.uniform(9,11,size=(num_tasks,))
        #masses=np.random.uniform(0.7,1.3,size=(num_tasks,))
        column_names=['length', 'force']#,'mass']
        dic=zip(lengths,forces)#,masses)
